	...
	public class LightingRewritten : MonoBehaviour
	{
		...
		private List<LightInteractVertex> raycastAtCollider(PolygonCollider2D collider)
		{
			...
			for (int i = 0; i < collider.GetTotalPointCount(); i++)
			{
				...
				if (ray)
				{ //If this hit anything at all (it's possible to miss when a collider has a point further than lightradius but has one within lightradius
		...
		{
			if (vert.hitIntendedPolygon)
			{ //If it was obstructed at the very edge angles, we don't need to bother
